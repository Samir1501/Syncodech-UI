...
namespace Syncodech.UI
{
    ...
    [SelectionBase]
    [ExecuteAlways]
    public class ScrollArea : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler,
        ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        private void LateUpdate()
        {
            ...
            if (_prevBaseSize != _baseRect.rect.size || _prevViewportSize != viewport.rect.size ||
        ...
        public void OnBeginDrag(PointerEventData eventData)
        {
            if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(viewport, eventData.position,
                    eventData.pressEventCamera, out _contentPosition)) return;
        ...
        {
            if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(viewport, eventData.position,
                    eventData.pressEventCamera, out Vector2 mousePosition)) return;
